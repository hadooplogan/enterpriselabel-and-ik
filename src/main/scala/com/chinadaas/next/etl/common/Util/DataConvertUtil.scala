...
object DataConvertUtil {
  ...
  def deletePath(hdfs: FileSystem, name: Path) = {
    hdfs.delete(name)
  }

  def deletePath(hdfs:FileSystem,name:String) = {
    hdfs.delete(new Path(name))
  }

  def mkdir(hdfs: FileSystem, name: Path) = {
    hdfs.mkdirs(name)
  }

  def mkdir(hdfs:FileSystem,name:String) = {
    hdfs.mkdirs(new Path(name))
  }
