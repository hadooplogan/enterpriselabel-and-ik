...
import Util.{DataFrameUtils, TimeUtil}
import org.apache.spark.sql.{DataFrame, SparkSession}


object BaseInfoKpi {
  def BaseIfo(spark: SparkSession): DataFrame = {

    val hql: String = " select a.pripid ,\n" +
      " a.credit_code ,\n" +
      " a.entname ,\n" +
      ...
      " when entstatus = '1' then round (((" + TimeUtil.getNowYear() + "-year(esdate))*12+" + TimeUtil.getNowMonth() + "-month(esdate))/12,1) \n" +
      " else '-9'\n" +
      " end as eb0004,\n" +
      " a.regcap as ee0004,\n" +
    ...
    DataFrameUtils.getDataFrame(spark, hql, "baseinfo")
